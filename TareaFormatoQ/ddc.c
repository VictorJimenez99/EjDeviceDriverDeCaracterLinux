...
long int *buffer;
long int func[MAX_SIZE];

extern short int ventana[MAX_SIZE];
...
static int driver_open(struct inode *inode, struct file *file)
{
	...
	buffer = (long int*)kmalloc(sizeof(long int) * MAX_SIZE, GFP_KERNEL);
